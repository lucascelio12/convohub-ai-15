...
CREATE POLICY "Managers can view their assigned queues" ON public.queues FOR SELECT USING (
  ...
  EXISTS (
    ...
    WHERE p.user_id = auth.uid() AND qm.queue_id = queues.id
  )
);
...
CREATE POLICY "Users can view conversations based on role" ON public.conversations FOR SELECT USING (
  ...
  (public.get_current_user_role() = 'manager' AND EXISTS (
    SELECT 1 FROM public.queue_managers qm
    INNER JOIN public.profiles p ON p.id = qm.manager_id
    WHERE p.user_id = auth.uid() AND qm.queue_id = conversations.queue_id
  )) OR
  (public.get_current_user_role() = 'agent' AND EXISTS (
    SELECT 1 FROM public.profiles p
    WHERE p.user_id = auth.uid() AND p.id = conversations.assigned_to
  ))
);
